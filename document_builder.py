...
document_builder.py
────────────────────
Converts a raw DB row → human-readable text for embedding.

Supports:
  • Handcrafted templates for known OFB tables (richer, more precise)
  • Generic fallback for auto-discovered tables (joins all text columns)

The text reads like a natural description so NL queries match well.
"""
...
def _extract_file_content(r) -> str:
    """Try to extract text from the file referenced in this row."""
...
def build_document(cfg: TableConfig, row: Dict[str, Any]) -> Tuple[str, str, Dict]:
    """
    Returns (doc_id, document_text, metadata_dict).

    For known tables: uses a handcrafted template (rich, precise text).
    For auto-discovered tables: generic fallback that joins all text_columns.
    """
    builder = _BUILDERS.get(cfg.table)
    if builder:
        text = builder(row)
    else:
        # Generic fallback for auto-discovered DBs
        text = cfg.label + "\n"
    ...
    # Metadata (only ChromaDB-compatible scalar types)
